...
export function SiteHeader() {
  ...
  return (
    ...
    >
      <div
        className={`mx-auto max-w-[1800px] px-8 lg:px-12 flex items-center justify-between transition-all duration-500 h-20`}
      >
        ...
        >
          ...
          <div className="space-y-1.5">
            <span
              className={`block h-0.5 w-5 bg-ink transition-transform duration-300 ${open ? "rotate-45 translate-y-2" : ""}`}
            />
            <span
              className={`block h-0.5 w-5 bg-ink transition-opacity duration-300 ${open ? "opacity-0" : ""}`}
            />
            <span
              className={`block h-0.5 w-5 bg-ink transition-transform duration-300 ${open ? "-rotate-45 -translate-y-2" : ""}`}
            />
